...
@mcp.tool()
def search_features(ctx: Context, query: str) -> str:
    """
    Sucht nach Features über alle Layer hinweg basierend auf einem Suchbegriff.
    Nutze dies für allgemeine Suchanfragen wie 'Finde Kreuzungen', 'Suche Hauptstraße' etc.
    
    Args:
        query: Suchbegriff oder Beschreibung (z.B. 'Kreuzungen in Hamburg', 'Hauptstraße')
    
    Returns:
        JSON mit gefundenen Features und Metadaten
    """
    qgis = get_qgis_connection()
    
    # Hole alle Layer
    layers_result = qgis.send_command("get_layers")
    
    if not layers_result or "layers" not in layers_result:
        return json.dumps({"error": "Could not retrieve layers", "features": [], "count": 0})
    
    all_features = []
    search_term = query.lower()
    
    # Durchsuche alle Layer
    for layer in layers_result["layers"]:
        layer_id = layer["id"]
        layer_name = layer["name"].lower()
        
        # Prüfe ob Layer-Name zum Suchbegriff passt
        if search_term in layer_name or any(term in layer_name for term in search_term.split()):
            # Hole Features aus diesem Layer
            features_result = qgis.send_command("get_layer_features", {
                "layer_id": layer_id,
                "limit": 50  # Max 50 Features pro Layer
            })
            
            if features_result and "features" in features_result:
                for feature in features_result["features"]:
                    feature["source_layer"] = layer["name"]
    ...
    logger.info(f"Search for '{query}' found {len(all_features)} features")
    return json.dumps(result, indent=2)


@mcp.tool()
def export_layer_geojson(ctx: Context, layer_id: str) -> str:
    """
    Export a QGIS layer as GeoJSON. Returns the GeoJSON content as a string.
    Use this to export layers for visualization in web maps or other applications.
    
    Args:
        layer_id: The ID of the layer to export
    
    Returns:
        JSON containing the GeoJSON data and metadata
    """
    qgis = get_qgis_connection()
    result = qgis.send_command("export_layer_geojson", {"layer_id": layer_id})
    return json.dumps(result, indent=2)
